...
class GfG
{
    ...
    // Function to convert a given BST into a balanced BST
    Node buildBalancedTree(Node root) 
    {
        // Collect nodes of the BST in sorted order using in-order traversal
        List<Node> nodes = new ArrayList<>();
        inOrderTraversal(root, nodes);

        // Convert the sorted nodes into a balanced BST
        int n = nodes.size();
        int[] sortedArr = new int[n];
        for (int i = 0; i < n; i++) {
            sortedArr[i] = nodes.get(i).data;
        }
        
        return sortedArrayToBST(sortedArr, 0, n - 1);
    }
